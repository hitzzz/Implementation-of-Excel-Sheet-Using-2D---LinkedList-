/*
LINKED LIST CLASS WRITTEN BY HITESH JHURANI
*/
...
public:
	...
	iterator erase(iterator position, int row) {
		...
		if (nullptr != position.location)
		{
			...
			if (row == 0)
			{
				//position.location->getPrev() = nullptr ;
	...
	iterator insert(iterator pos, const T& value, int row) {
