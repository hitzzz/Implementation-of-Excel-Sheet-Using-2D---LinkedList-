...
if %ERRORLEVEL% GEQ 1 (
    ...
    ECHO middle row, ECHO and finally adding a new first row
	ECHO ********************************************************
	...
	if ERRORLEVEL 0 (
		...
		findstr /R /C:"^[ 	]*,[	 ]*,[	 ]*!CR!!LF![	 ]*,[	 ]*,[	 ]*!CR!!LF![ 	]*,[	 ]*-13.0[0]*,[	 ]*test!CR!!LF!3.14[0]*,[	 ]*,[	 ]*$" temp.txt > NUL
		if ERRORLEVEL 1 (
			echo -----Failed-----
		) else (
			echo +++++Passed+++++
	...
	if ERRORLEVEL 0 (
		REM Allow cells to be separated by tabs or spaces with LF/CR return statements separating the rows
		findstr /R /C:"^[	 ]*,[	 ]*-13.0[0]*,[	 ]*test$" temp.txt > NUL
		if ERRORLEVEL 1 (
			echo -----Failed-----
		) else (
			echo +++++Passed+++++
